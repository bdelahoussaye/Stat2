...
% Auto-generated by MATLAB on 05-Dec-2018 21:10:46
...
predictorNames = {'Height', 'Weight', 'YearStart', 'YearEnd', 'Position', 'PositionsPlayed', 'College', 'GP', 'MIN', 'PTS', 'FGM', 'FGA', 'FGPer', 'X3PMade', 'X3PA', 'X3Per', 'FTM', 'FTA', 'FTPer', 'OREB', 'DREB', 'REB', 'AST', 'STL', 'BLK', 'TOV', 'School_power', 'Birth_Month', 'GP_level', 'MIN_level'};
predictors = inputTable(:, predictorNames);
response = inputTable.TARGET_5Yrs;
isCategoricalPredictor = [false, false, false, false, true, false, true, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, true, true, true, true];

% Data transformation: Select subset of the features
% This code selects the same subset of features as were used in the app.
includedPredictorNames = predictors.Properties.VariableNames([true true true false true true true true true true true true true true true true true true true true true true true true true true true true true true]);
predictors = predictors(:,includedPredictorNames);
isCategoricalPredictor = isCategoricalPredictor([true true true false true true true true true true true true true true true true true true true true true true true true true true true true true true]);
...
trainedClassifier.predictFcn = @(x) logisticRegressionPredictFcn(pcaTransformationFcn(featureSelectionFcn(predictorExtractionFcn(x))));

% Add additional fields to the result struct
trainedClassifier.RequiredVariables = {'Height', 'Weight', 'YearStart', 'YearEnd', 'Position', 'PositionsPlayed', 'College', 'GP', 'MIN', 'PTS', 'FGM', 'FGA', 'FGPer', 'X3PMade', 'X3PA', 'X3Per', 'FTM', 'FTA', 'FTPer', 'OREB', 'DREB', 'REB', 'AST', 'STL', 'BLK', 'TOV', 'School_power', 'Birth_Month', 'GP_level', 'MIN_level'};
...
for fold = 1:KFolds
    ...
    % Data transformation: Select subset of the features
    % This code selects the same subset of features as were used in the app.
    includedPredictorNames = trainingPredictors.Properties.VariableNames([true true true false true true true true true true true true true true true true true true true true true true true true true true true true true true]);
    trainingPredictors = trainingPredictors(:,includedPredictorNames);
    foldIsCategoricalPredictor = foldIsCategoricalPredictor([true true true false true true true true true true true true true true true true true true true true true true true true true true true true true true]);
    ...
    pcaTransformationFcn = @(x) [ array2table((table2array(varfun(@double, x(:, ~isCategoricalPredictorBeforePCA))) - pcaCenters) * pcaCoefficients), x(:,isCategoricalPredictorBeforePCA) ];
    logisticRegressionPredictFcn = @(x) predictionsAndScoresFcn( predict(GeneralizedLinearModel, x) );
    validationPredictFcn = @(x) logisticRegressionPredictFcn(pcaTransformationFcn(featureSelectionFcn(x)));
